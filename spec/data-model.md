...
保存 UUID、名称、最终描述、媒体类型、上传时文件名、正规化后媒体的相对路径/MIME/大小、直接入库标志、处理状态、审核状态、失败信息和审计时间。
...
视频素材目录中的 `frames/manifest.json` 保存正规化后视频时长、1–5 个 JPEG 帧文件名及对应时间点。它不单独写入数据库；分析重试从该清单复用关键帧，软删除后的 cleanup 任务随素材目录一并移除。
