...
- [x] T005 建立 Drizzle MySQL schema、TLS/UTC 连接池和迁移
- [x] T006 实现安全本地存储、临时文件和原子移动
- [x] T007 实现图片签名/元数据与视频容器/编码校验
...
## Phase 11：API v1、MySQL、分镜与对象存储

- [x] T056 统一 `/api/v1` 三步上传与统一任务查询，移除旧上传状态标识和旧路由
- [x] T057 切换到远程 MySQL 8，并将仓储、任务抢占和处理链全部异步化
- [x] T058 增加 `user_id` 公共/个人素材作用域和受控组合查询
- [x] T059 接入同机分镜服务，实施切片 10 MiB 与整批全有或全无边界
- [x] T060 接入私有 ZOS、Range 直读、持久任务回调和 24 小时 staging 清理
- [x] T061 统一可信内网无鉴权访问，重写 OpenAPI 和 API 文档
